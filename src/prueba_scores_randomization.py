# -*- coding: utf-8 -*-
"""
Created on Sat Mar 27 16:20:11 2021
...
if __name__ == "__main__":
    ...
    Path("../data/model_build_outputs/scores").mkdir(parents=True, exist_ok=True)
    randomization = int(sys.argv[1])
    
    
    df_results = pd.DataFrame(None)
    #df_results.at[0, 'route_sequence'] = 0
    df_results.at[0, 'evaluation_score'] = 0
    new_routes_dict = {}
    na_values = [""]
    number_of_routes = len(listdir("../data/model_build_outputs/grasp_routes_prob_random_"+str(randomization)))
    ...
    for i in range(number_of_routes):
        try:
            ...
            #with open("../grasp_routes_lexicografico/solution_route_"+str(i)+normalized+"_alpha_"+str(alpha)+"_beta_"+str(beta)+grasp_type, 'r') as read_obj:
            ...
            print("Here 2")
            travel_times = pd.read_csv("../data/model_build_outputs/travel_times_files/travel_times_route_"+str(i)+".csv", na_values=na_values, keep_default_na=False)
            print("Here 3")
            
            #Convertir travel_time_matrix al formato de ellos (dict)
            new_mat = travel_times.set_index('key')
            cost_mat = new_mat.to_dict()
            #Para que funcione en el notebook, hay que incluir al inicio y al final de la ruta el depot
    ...
    df_results['randomization'] = randomization
    
    final_mean_score = df_results.evaluation_score.mean()
    df_results['final_mean_score'] = final_mean_score
    df_results.to_csv("../data/model_build_outputs/scores/df_results_prob_random_"+str(randomization)+".csv")
    #df_results.to_csv('df_results_prueba_grasp_por_fases_permutaciones.csv')
    #df_5 = pd.DataFrame(new_routes_dict)
    #df_5.to_csv('dict-or-tools-results.csv', encoding='utf-8',)
