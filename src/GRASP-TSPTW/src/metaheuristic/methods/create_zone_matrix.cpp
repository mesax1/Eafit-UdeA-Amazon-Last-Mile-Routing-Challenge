...
TSPZoneMatrixSolution create_zones_matrix(TSPInstance& instance){
    ...
    auto* solution = new TSPSolution(instance, 1);
    auto* zone_solution = new TSPSolution(instance, 1);
    ...
    //CREATE MATRICES THAT WILL HOLD INFORMATION ABOUT AVERAGE TRAVEL TIME  BETWEEN ZONES, AND AVERAGE HAVERSINE DISTANCE BETWEEN ZONES
    vector<vector<float>> zones_time_matrix(ordered_zones_map.size(), vector<float>(ordered_zones_map.size(), 0));
    vector<vector<float>> zones_distance_matrix(ordered_zones_map.size(), vector<float>(ordered_zones_map.size(), 0));
    ...
    float max_haversine_distance = 1.0;
    float max_nan_haversine_distance = 1.0;
    ...
    for (int h=0; h < ordered_zones_map.size(); h++) {
        for (int k=0; k < ordered_zones_map.size(); k++) {
                ...
                if (solution->customers[i]->zone_id == zone_itr_1->first) {
                    ...
                    for (int j = 0; j < solution->customers.size(); j++) {
                        if (solution->customers[j]->zone_id == zone_itr_2->first){
                            ...
                            total_distance_rows += instance.distance_matrix[a->id][b->id];
                            customers_of_zone += 1;
                        }
                    }
                }
            ...
            if (zone_itr_1->first == zone_itr_2->first){
                ...
                zones_distance_matrix[h][k] = 0;
            }else {
                //AVERAGE THE SUMS OF TIMES AND DISTANCES, ACCORDING TO THE NUMBER OF CUSTOMERS IN EACH ZONE
                zones_time_matrix[h][k] = total_time_rows / customers_of_zone;
                zones_distance_matrix[h][k] = total_distance_rows / customers_of_zone;
                ...
                if (zones_distance_matrix[h][k] >= max_haversine_distance){
                    if ((zone_itr_1->first != "nan")&&(zone_itr_2->first != "nan")) {
                        max_haversine_distance = zones_distance_matrix[h][k];
                    }
                }
                if (zones_distance_matrix[h][k] >= max_nan_haversine_distance){
                    max_nan_haversine_distance = zones_distance_matrix[h][k];
                }
            }
            zone_itr_2++;
        }

        TSPCustomer* c = new TSPCustomer(h, {-1, -1}, 0, 0, zone_itr_1->first );
    ...
    for (int h=0; h < ordered_zones_map.size(); h++) {
        for (int k=0; k < ordered_zones_map.size(); k++) {
            if ((zone_itr_1->first != "nan")&&(zone_itr_2->first != "nan")) {
                ...
                instance.zones_distance_matrix_df[zone_itr_1->first][zone_itr_2->first] = zones_distance_matrix[h][k] / max_haversine_distance;
            }
            else{
                ...
                instance.zones_distance_matrix_df[zone_itr_1->first][zone_itr_2->first] = zones_distance_matrix[h][k] / max_nan_haversine_distance;
            }
            zone_itr_2++;
        }
        zone_itr_1++;
