...
TSPSolution* run_vnd_generator(TSPSolution& initial_solution,
...
{
    TSPSolution* best_solution = new TSPSolution(initial_solution);
    float previous_cost = best_solution->compute_prob_cost(instance);
    int neighborhood = 0;
    float current_cost = 0;
    while(neighborhood < neighborhoods.size()) {
        TSPSolution* local_optima = neighborhoods[neighborhood]->search_neighbors(best_solution, instance, by_zones);
        ...
        if(current_cost < previous_cost) {
            ...
            local_optima_output << local_optima->TSP_output_string_solutions(instance).str()  << endl;
...
TSPSolution* run_prob_tsp_vnd(TSPSolution& initial_solution,
                  const TSPInstance& instance,
                  vector<TSPNeighborhood*>& neighborhoods,
                  bool by_zones
                  )
