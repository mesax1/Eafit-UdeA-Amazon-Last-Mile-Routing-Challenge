...
TSPHistoricValues read_historic_files(const string& route_fid, const int& number_of_zones, const int& number_of_customers, const int& in_test_phase) {
    ...
    string distance_matrix_file( "../haversine_distance_matrices/"+route_fid+"_dist_matrix.csv" );
    ...
    if (in_test_phase == 1){
        ...
        historic_zones_file = "../data/model_apply_outputs/prob_matrices_heading/"+route_fid+"_zroutes.csv" ;
        distance_matrix_file = "../data/model_apply_outputs/haversine_distance_matrices/"+route_fid+"_dist_matrix.csv" ;
    }
    else {
        ...
        historic_zones_file = "../data/model_build_outputs/prob_matrices_heading/"+route_fid+"_zroutes.csv" ;
        distance_matrix_file = "../data/model_build_outputs/haversine_distance_matrices/"+route_fid+"_dist_matrix.csv" ;
    }
    ...
    vector<string> elements_6 = read_matrix_csv(distance_matrix_file); \
    vector<string>::iterator ptr_6 = elements_6.begin();
    ...
    vector<vector<float>> distance_matrix(number_of_customers, vector<float>(number_of_customers, 0));
    ...
    //READ HAVERSINE DISTANCE MATRIX

    cout << "number_of_customers: " << number_of_customers << endl;
    for(int i = 0; i < number_of_customers; i++){
        ...
        for(int j = 0; j < number_of_customers; j++){
            distance_matrix[i][j] = stof(*next(ptr_6, 1));
            advance(ptr_6, 1);
        }
    }
    cout << "Parsed DISTANCE MATRIX correctly" << endl;
    ...
    return {probabilities_matrix, n_routes_matrix, historic_zones_id, probabilities_df, angles_df, n_routes_df, max_n_routes, distance_matrix};
}
